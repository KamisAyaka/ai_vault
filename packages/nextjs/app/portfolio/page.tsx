    ...
    <>
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="card-body">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="btn-group">
                <button
                  className={`btn btn-sm ${selectedPeriod === "7d" ? "bg-[#803100] text-white" : "bg-black/60 text-[#fbe6dc] border-[#803100]/30"}`}
                  onClick={() => setSelectedPeriod("7d")}
                >
                  {t("revenue.period.7d")}
                </button>
                <button
                  className={`btn btn-sm ${selectedPeriod === "30d" ? "bg-[#803100] text-white" : "bg-black/60 text-[#fbe6dc] border-[#803100]/30"}`}
                  onClick={() => setSelectedPeriod("30d")}
                >
                  {t("revenue.period.30d")}
                </button>
                <button
                  className={`btn btn-sm ${selectedPeriod === "90d" ? "bg-[#803100] text-white" : "bg-black/60 text-[#fbe6dc] border-[#803100]/30"}`}
                  onClick={() => setSelectedPeriod("90d")}
                >
                  {t("revenue.period.90d")}
                </button>
                <button
                  className={`btn btn-sm ${selectedPeriod === "all" ? "bg-[#803100] text-white" : "bg-black/60 text-[#fbe6dc] border-[#803100]/30"}`}
                ...
                {filteredRevenueHistory.map((point, index) => {
                  ...
                  return (
                    <div key={point.timestamp} className="flex flex-col items-center flex-1 group relative">
                      ...
                      ></div>
                      {index % 5 === 0 && (
                        <p className="text-xs mt-2 text-[#fbe6dc] rotate-45 origin-top-left">{point.date}</p>
                      )}
                  ...
                  <tbody>
                    {positions.map(position => (
                      <tr
                        ...
                        className="portfolio-position-row border-b border-[#803100]/30 hover:bg-[#803100]/10"
                      >
                        <td>
                          <Link
                            href={`/vaults/${position.vault.id}`}
                            className="font-semibold text-white hover:text-[#803100]"
                          >
                            {position.vault.name}
                          </Link>
                        </td>
                        <td>
                          <span className="badge bg-[#803100]/20 border-[#803100]/30 text-[#fbe6dc]">
                        ...
                        <td className="text-sm text-[#fbe6dc]">
                          {Math.round(position.daysHeld)} {t("positions.table.days")}
                        </td>
                        <td>
                          <Link
                            href={`/vaults/${position.vault.id}`}
                            className="btn btn-xs bg-[#803100] hover:bg-[#803100]/80 border-none text-white"
                          >
                  ...
                  <tbody>
                    {feeBreakdown.map(row => (
                      <tr
                        ...
                        className="portfolio-fee-row border-b border-[#803100]/30 hover:bg-[#803100]/10"
                      >
                    ...
                    <tr className="font-bold border-t-2 border-[#803100]/30">
                      ...
                      <td className="text-[#fbe6dc]">
                        $
                        {formatCurrency(
                          feeBreakdown.reduce((sum, row) => sum + row.managementFee, 0),
                          2,
                        )}
                      </td>
                      <td className="text-[#fbe6dc]">
                        $
                        {formatCurrency(
                          feeBreakdown.reduce((sum, row) => sum + row.performanceFee, 0),
                          2,
                        )}
                      </td>
        ...
        {transactionHistory && transactionHistory.length > 0 && (
          <div
            className="portfolio-section-card card bg-black/60 backdrop-blur-sm shadow-xl border border-[#803100]/30"
            ref={activityRef}
          >
            <div className="card-body">
                ...
                <table className="table table-sm">
                  ...
                  <tbody>
                    {transactionHistory.map(tx => (
                      <tr
                        key={tx.id}
                        className="portfolio-activity-row border-b border-[#803100]/30 hover:bg-[#803100]/10"
                      >
                        <td className="text-xs text-[#fbe6dc]">{new Date(tx.timestamp).toLocaleString("zh-CN")}</td>
                        <td>
                          <span
                            className={`badge badge-sm ${tx.type === "deposit" ? "bg-green-400/20 border-green-400/30 text-green-400" : "bg-red-400/20 border-red-400/30 text-red-400"}`}
                          >

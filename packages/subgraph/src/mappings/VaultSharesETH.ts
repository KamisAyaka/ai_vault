...
import { IERC20Metadata } from "../../generated/VaultSharesETH/IERC20Metadata";
import {
  Vault,
  User,
  UserStats,
...
function getAssetAddressFromVault(vaultAddress: Address): Address {
  let vaultContract = VaultSharesETH.bind(vaultAddress);
...
export function handleDeposit(event: Deposit): void {
  ...
  if (!vault) {
    ...
    // 从Vault合约获取信息
    let vaultContract = VaultSharesETH.bind(event.address);

...
contract RealProtocolIntegrationTest is Test {
    ...
    function _createVault() internal {
        vm.prank(owner);
        vaultAddress = manager.createVault(usdc);
    ...
    function _testManagerParameterAdjustments() internal {
        ...
        string memory name3 = abi.decode(results[2], (string));

        assertEq(name1, "Aave", "First adapter name should match");
        assertEq(name2, "UniswapV2", "Second adapter name should match");
        assertEq(name3, "UniswapV3", "Third adapter name should match");

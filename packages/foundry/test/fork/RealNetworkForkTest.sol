...
contract RealNetworkForkTest is Test {
    ...
    // 真实协议地址 (Mainnet)
    address public constant AAVE_POOL_MAINNET =
        0x87870Bca3F3fD6335C3F4ce8392D69350B4fA4E2;
    address public constant UNISWAP_V2_ROUTER_MAINNET =
        0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address public constant UNISWAP_V3_ROUTER_MAINNET =
        0xE592427A0AEce92De3Edee1F18E0157C05861564;
    address public constant UNISWAP_V3_POSITION_MANAGER_MAINNET =
        0xC36442b4a4522E871399CD717aBDD847Ab11FE88;
    address public constant UNISWAP_V3_FACTORY_MAINNET =
        0x1F98431c8aD98523631AE4a59f267346ea31F984;
    address public constant UNISWAP_V3_QUOTER_MAINNET =
    ...
    function testMainnetIntegration() public {
        ...
        // 测试基本集成 - 只验证合约部署和配置
        console.log(
            "Basic integration test completed - all contracts deployed and configured"
        );
    ...
    function testUniswapV2ProtocolIntegration() public {
        ...
        _deployManager();
        uniswapV2Adapter = new UniswapV2Adapter(UNISWAP_V2_ROUTER_MAINNET);
        ...
        // 验证投资
        uint256 totalValue = uniswapV2Adapter.getTotalValue(
            IERC20(USDC_MAINNET)
        );
    ...
    function testUniswapV3WBTCSmartRangeIntegration() public {
        vm.selectFork(mainnetFork);
        console.log(
            "=== Testing UniswapV3 WBTC/USDC Smart Range Integration ==="
        );
        ...
        // 先查询当前池子的价格信息
        address poolAddress = IUniswapV3Factory(UNISWAP_V3_FACTORY_MAINNET)
            .getPool(WBTC_MAINNET, USDC_MAINNET, 3000);
        ...
        vm.prank(vaultAddress);
        uint256 investedAmount = uniswapV3Adapter.invest(
            IERC20(WBTC_MAINNET),
            1 * 1e8
        );
        ...
        // 验证投资
        uint256 totalValue = uniswapV3Adapter.getTotalValue(
            IERC20(WBTC_MAINNET)
        );
    ...
    function testUniswapV3ProtocolIntegration() public {
        ...
        // 先查询当前池子的价格信息
        address poolAddress = IUniswapV3Factory(UNISWAP_V3_FACTORY_MAINNET)
            .getPool(USDC_MAINNET, WETH_MAINNET, 3000);
        require(poolAddress != address(0), "Pool does not exist");
    ...
    function testMultiProtocolInvestment() public {
        ...
        vm.prank(user);
        VaultShares(vaultAddress).deposit(10000 * 1e6, user);
    ...
    function _createVault() internal {
        vaultAddress = manager.createVault(IERC20(USDC_MAINNET));
    ...
    function _testInvestmentFlow() internal {
        ...
        VaultShares(vaultAddress).deposit(1000 * 1e6, user);
    ...
    function _testAssetManagement() internal view {
        ...
        // 测试资产查询 - 通过金库合约查询总资产
        uint256 totalValue = VaultShares(vaultAddress).totalAssets();
    ...
    function _fundUserWithToken(address token, uint256 amount) internal {
        ...
        if (whaleBalance >= amount) {
            ...
            console.log(
                "Warning: Whale doesn't have enough tokens for user funding"
            );
    ...
    function _fundVaultWithToken(address token, uint256 amount) internal {
        ...
        if (whaleBalance >= amount) {
            ...
            console.log(
                "Warning: Whale doesn't have enough tokens, skipping funding"
            );
            console.log("Whale has:", whaleBalance, "but need:", amount);
        }
    }

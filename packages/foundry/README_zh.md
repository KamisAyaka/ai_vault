...
graph LR
    ...
    linkStyle 9 stroke:#FF69B4,stroke-width:2px
    ...
    class I,J,K protocol;
```

## 工作流程

1. **金库创建**：管理者通过 AIAgentVaultManager 创建 VaultShares 金库
...
4. **费用管理**：收取 1%管理费用并分配给协议所有者
5. **紧急停止**：在紧急情况下停止金库操作
6. **动态调整**：支持部分或全部更新投资策略
7. **资产估值**：实时计算总资产管理规模（TVL）

### 协议适配器功能

1. **统一接口**：为不同 DeFi 协议提供标准化接口（invest, divest, getTotalValue）
2. **投资操作**：支持投资（invest）和撤资（divest）操作
3. **价值评估**：实时计算在协议中的资产价值
4. **参数配置**：支持配置特定协议参数（如滑点容忍度、配对代币、价格区间等）
5. **权限控制**：确保只有授权的金库可以调用适配器函数

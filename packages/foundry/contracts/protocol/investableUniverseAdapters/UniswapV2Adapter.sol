...
contract UniswapV2Adapter is IProtocolAdapter, Ownable {
     ...
     * @param counterPartyToken 配对代币
     */
    function updateTokenConfigAndReinvest(
        IERC20 token,
        IERC20 counterPartyToken
    ) external onlyOwner {
        // 验证输入参数
        ...
        // 更新配置并重新投资所有可用资产
        // 计算新的pair接口实例
        address newPairAddress = i_uniswapFactory.getPair(
            address(token),
            address(counterPartyToken)
        );
    ...
    function _invest(IERC20 asset, uint256 amount) internal returns (uint256) {
        ...
        // 执行兑换并获取实际兑换数量
        uint256 expectedTokenB = i_uniswapRouter.getAmountsOut(
            amountOfTokenToSwap,
            path
        )[1];
        uint256 minTokenB = (expectedTokenB *
    ...
    function _divest(
        ...
        // 批准 router 转移 LP 代币
        IERC20(address(pair)).forceApprove(
            address(i_uniswapRouter),
            liquidityToRemove
        );
        ...
        // 执行流动性移除
        (uint256 actualTokenAmount, uint256 counterPartyAmount) = i_uniswapRouter
            .removeLiquidity({
                tokenA: address(asset),
                tokenB: address(config.counterPartyToken),
    ...
    function getTotalValue(
        ...
        uint256 liquidityTokens = IERC20(address(pair)).balanceOf(
            address(this)
        ); // 查询适配器的LP代币余额

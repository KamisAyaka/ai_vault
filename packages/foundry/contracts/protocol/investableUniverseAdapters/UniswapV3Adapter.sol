...
import { INonfungiblePositionManager } from
    "../../vendor/UniswapV3/periphery/interfaces/INonfungiblePositionManager.sol";
...
contract UniswapV3Adapter is IProtocolAdapter, Ownable {
     ...
     */
    function UpdateTokenFeeTierAndPriceRange(IERC20 token, uint24 newFeeTier, int24 newTickLower, int24 newTickUpper)
        external
        onlyOwner
    {
        TokenConfig memory config = getTokenConfig(token);
        ...
        (uint128 liquidityMinted, uint256 newAmount0, uint256 newAmount1) = _investWithBalances(token, config);

        emit UniswapV3Invested(address(token), newAmount0, newAmount1, liquidityMinted);
    }
     ...
     */
    function _investWithBalances(IERC20 token, TokenConfig memory config)
        internal
        returns (uint128 liquidityMinted, uint256 amount0, uint256 amount1)
    {
        ...
        (uint256 newAmount0Desired, uint256 newAmount1Desired) =
            LiquidityAmounts.getAmountsForLiquidity(sqrtPriceX96, sqrtRatioAX96, sqrtRatioBX96, newLiquidity);
        ...
        if (balance0 < newAmount0Desired || balance1 < newAmount1Desired) {
            ...
            if (balance0 < newAmount0Desired && balance1 > newAmount1Desired) {
                ...
                _swapToken(
                    IERC20(token1Addr), IERC20(token0Addr), config.feeTier, amountToSwap, config.slippageTolerance
                );
            } else if (balance1 < newAmount1Desired && balance0 > newAmount0Desired) {
                // 需要更多token1，有多余的token0
                uint256 amountToSwap = newAmount1Desired - balance1;
                _swapToken(
                    IERC20(token0Addr), IERC20(token1Addr), config.feeTier, amountToSwap, config.slippageTolerance
                );
            }
        ...
        INonfungiblePositionManager.MintParams memory mintParams = INonfungiblePositionManager.MintParams({
            ...
            amount0Min: (balance0 * (BASIS_POINTS_DIVISOR - config.slippageTolerance)) / BASIS_POINTS_DIVISOR,
            amount1Min: (balance1 * (BASIS_POINTS_DIVISOR - config.slippageTolerance)) / BASIS_POINTS_DIVISOR,
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        // 计算最小输出量以提供滑点保护
        uint256 amount0Min = (amount0Desired * (BASIS_POINTS_DIVISOR - config.slippageTolerance)) / BASIS_POINTS_DIVISOR;
        uint256 amount1Min = (amount1Desired * (BASIS_POINTS_DIVISOR - config.slippageTolerance)) / BASIS_POINTS_DIVISOR;
